...
薄 Runner 不等于完全无状态：请求关联、后端实例身份、XCTest 所需的线程/执行顺序以及一次调用所需的临时状态可以保留。type/press 保留实时 AX 目标检查；tap/swipe 使用宿主传入的坐标与分区截图基准，后端进行通用上下文和画面比较后直接执行，不保存跨请求快照或重新按名称查找目标。将引用管理移到宿主不能使旧坐标自动变得可靠，近似比较的边界见 [画面校验契约](screen-guard.md)。

动作失效的宿主侧实现：在同一设备的串行临界区内，先校验并解析引用，将它置于不可复用的待定状态，再向后端派发。动作已派发或结果未知时旧引用失效；只有能确认未派发且没有其他失效原因时才解除待定状态。宿主进程重启后不继承旧引用的有效性，也不自动重放未完成请求。这样无需 Runner 实现面向 agent 的引用协议，仍能覆盖“点击已执行但调用方未收到响应”的情况。当前 open/tap/swipe/type/press 已接入此流程；Runner 根据动作类型校验 AX 目标或坐标画面前置条件并返回执行事实。画面或目标变化即使未派发也使引用失效；unknown 不重发，见 [动作契约](actions.md)。
