...
| `swipe` | 在指定元素区域按明确方向，或在观察中的明确端点之间滑动；可指定速度、移动前按住及抬手前停留；输入前进行画面校验。 |
...
| `outcome` | 含义 | 例子 |
| --- | --- | --- |
| `completed` | `act` 的全部输入调用正常完成，且动作后帧 hash 达到稳定。它不等于用户目标达成。 | 点击后的画面稳定；登录是否成功仍需 agent 核对。 |
...
动作结果现在包含执行事实、帧稳定状态和最后一帧的本地路径。调用 agent 用该图像核对结果，显式调用 `observe` 获取新的 AX 和有效引用。动作帧不包含动作前的 AX，也不创建新的元素引用。

**2026-09-05 已接入的完成契约**：`act` 的输入事件执行结束后，由 AgentSoma 连续采集屏幕帧、比较帧 hash，直到满足帧稳定条件，才将其报告为本次动作完成。检测是设备动作层的确定性工作，不需要调用 agent 逐帧判断或添加固定长 sleep。`replace` 等组合动作在全部输入原语执行结束后检测稳定，不能在中间点击或全选后提前结束。

已确认的流程为 `输入执行结束 → 采帧 / 比较 hash → 帧稳定 → 动作完成`。它定义本次动作结束时的可见状态；界面暂时稳定不能证明后续网络请求已经完成，业务目标仍由调用 agent 核对。Runner 同时将每个输入原语中的 XCTest application-state 等待上限暂设为 1 秒，调用结束后恢复原值；这不是整个动作的耗时承诺。启动时检查所需运行时符号，不支持则明确拒绝连接。

当前工程参数（具体数值由实现选择）：

- 按图像方向归一化，以全分辨率 sRGB RGBA8 像素和尺寸计算 SHA-256；不对 PNG 编码字节比较，不屏蔽画面区域，不以感知相似替代精确一致。Mac 端重新计算并校验最终图像的 hash。
- 首次采样在最后一个输入事件执行完成之后开始；以约 200ms 的间隔采样，至少 3 帧 hash 相同且稳定窗口覆盖至少 400ms。发生变化就重置窗口，以单调时钟记录时间；截图和处理超过采样间隔时不额外睡眠。
- 不要求画面一定先变化：对当前已选状态点击或同值替换可能没有视觉变化，仍可达到稳定。检测只能证明所观测窗口稳定，不能保证未来画面不会变化。
- 帧检测预算为 5 秒。到期返回 `outcome=unknown`、`error.code=frame_stability_timeout`，保留 `result.execution.inputCompleted=true`、`result.stability.stable=false` 和最后一帧。不得自动重放。预算在采帧/处理之间检查，无法中断正在阻塞的 XCTest 截图 API；传输超时且未收到响应时仍只能报告未知。
- 稳定帧是动作的结果帧。若同时返回或缓存界面结构，必须在动作后采集并标记各自时间，不能混用动作前的 AX；图像与 AX 仍不宣称原子一致。动作前引用照常失效。

本地测试覆盖稳定窗口重置、无视觉变化、单像素变化、PNG 元数据差异、持续变化和超时后的输入事实/引用处理。真机验收记录见 [动作接口](actions.md)。光标闪烁等真实变化若使全帧无法稳定，按超时返回，不静默弱化标准。`open` 继续使用 App 启动/激活的执行事实，不纳入本次 `act` 帧检测契约。
