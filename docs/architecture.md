...
The plugin contains instructions and a preflight script; it does not add another runtime service. Edit `skills/agentsoma/`, then run `python3 scripts/sync-plugin.py` to update the committed distribution. CI rejects drift. The [public website](https://agentsoma.hughlee824.chatgpt.site) is maintained in the separate [agentsoma-website](https://github.com/HughLee824/agentsoma-website) repository with deployment configured for Cloudflare Workers Static Assets at `agentsoma.dev`. The current Sites entry remains active until the Cloudflare cutover is verified. The public client installation path is documented in [public access release](public-access-release.md).
