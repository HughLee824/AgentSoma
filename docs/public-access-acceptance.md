...
The new repository owns its pinned Wrangler dependency, build checks and custom-domain configuration. Cloudflare Workers Builds is connected to its `main` branch; non-production builds are disabled. Before the first build, the dedicated token was narrowed to Workers Scripts edit and Account Settings read for the deployment account, User Details and Memberships read, and Zone read plus Workers Routes edit restricted to `agentsoma.dev`. Unrelated storage, database, AI, email and certificate permissions were removed. The original public Sites URL remains available.

Pushing website commit `3b8de7773613212b9d3a56c05ce3cb18dc54c4d4` triggered [a successful website CI run](https://github.com/HughLee824/agentsoma-website/actions/runs/34222283600) and Cloudflare build `eb21008f-24e9-40fa-9217-55079f3c4a5d`. The Cloudflare build used the narrowed token, ran the tests, all seven documentation-link checks and the static build, then deployed Worker version `2f35c82f-c4b3-462a-b560-87377f96edde` successfully. The production deployment showed this version receiving 100% of traffic. Both public homepages still matched the current build and returned 200; an unknown path returned 404.
