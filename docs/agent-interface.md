...
状态：需求与接口草案，完整协议尚未实现。2026-09-05 已实现 connect、status、open、observe、inspect、disconnect 及会话宿主，生命周期、观察、缓存与引用失效经过本地和真机验证；完整动作与实时目标校验仍待开发。具体观察格式见 [观察验收](observations.md)。实际可运行范围见 [使用说明](../README.md) 和 [宿主验收](session-host.md)。除明确标注已确认的方向外，以下工具名称、参数组织和默认行为是建议。
...
已确认采用短元素引用的方向，让 agent 无需重复填写长 identifier 或坐标。引用绑定观察与来源节点，不是现有 index，也不自动解决匹配歧义或保证目标仍存在；执行前必须能确认目标对应关系，不能确认则返回需重新观察。动作后的失效规则及 CLI 引用形式已确认；宿主引用生成、解析和失效已实现，后端实时目标定位仍待接入动作阶段。
...
本草案不是对现有 spike API 的直接改名。当前 open 已改为激活运行中的 App，并提供三态结果；Runner 未提供精确执行阶段的错误保守归为 unknown。观察已整合可确认的已打开 App、唯一 SpringBoard Alert 及未知前台时的截图；设备/App 发现、执行前目标校验及其余动作仍待实现。尤其是未知前台身份，现有实验尚未解决，应如实暴露限制。

首版 agent 入口为 CLI，最小 Swift 宿主已使用 Foundation 进程管理、Unix 域套接字与 JSON、ArgumentParser 落地；CoreDevice 直连及生命周期已在当前环境验证。自有薄 XCTest Runner 是当前设备后端。宿主观察与引用已通过；接下来实现完整动作与目标校验，见 [v0.1 实现计划](v0.1-plan.md)。
