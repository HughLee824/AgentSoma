...
状态：需求与架构对比，2026-09-05。用户已接受 CLI + 按会话运行的 Mac 宿主进程 + 薄 Runner 的结构、Swift 技术栈及空闲回收规则。CoreDevice 原生直连及最小 CLI / 宿主 IPC / 生命周期现已通过验证；宿主观察、缓存、引用与 inspect 也已实现，完整动作接口接着开发。见 [观察验收](observations.md)。见 [宿主验收](session-host.md)。
...
动作失效的宿主侧实现建议：在同一设备的串行临界区内，先校验并解析引用，将它置于不可复用的待定状态，再向后端派发。动作已派发或结果未知时旧引用失效；只有能确认未派发且没有其他失效原因时才解除待定状态。宿主进程重启后不继承旧引用的有效性，也不自动重放未完成请求。这样无需 Runner 实现面向 agent 的引用协议，仍能覆盖“点击已执行但调用方未收到响应”的情况。当前 open 已接入该宿主状态流程，本地测试验证未派发保留、正常派发失效和 unknown 不重发；按引用动作的实时设备校验留在下一阶段。
