...
当前决定：首版使用 CLI 与按会话运行的 Mac 宿主进程，保持设备会话复用，优先 CoreDevice 直连并尽量少依赖第三方工具；Runner 尽量薄且可被替换。设备通信、宿主观察与动作已实现，设备/App 发现与完整调用验收按后续计划完成。
...
动作失效的宿主侧实现：在同一设备的串行临界区内，先校验并解析引用，将它置于不可复用的待定状态，再向后端派发。动作已派发或结果未知时旧引用失效；只有能确认未派发且没有其他失效原因时才解除待定状态。宿主进程重启后不继承旧引用的有效性，也不自动重放未完成请求。这样无需 Runner 实现面向 agent 的引用协议，仍能覆盖“点击已执行但调用方未收到响应”的情况。当前 open/tap/swipe/type 已接入此流程；Runner 校验来源路径和目标属性并返回执行事实。歧义拒绝保留引用、目标变化使引用失效和 unknown 不重发均有验证，见 [动作契约](actions.md)。
