...
状态：需求与架构对比，2026-09-05。用户已接受 CLI + 按会话运行的 Mac 宿主进程 + 薄 Runner 的结构、Swift 技术栈及空闲回收规则。CoreDevice 原生直连及最小 CLI / 宿主 IPC / 生命周期现已通过验证；观察、引用与完整动作接口接着实现。见 [宿主验收](session-host.md)。
...
因此，短命 CLI 进程可以服务于持久设备会话。当前证明不是“没有持续运行的进程”：`start`、`iproxy`、`xcodebuild` 和 iPhone Runner 在会话期间持续存活。现有 CLI 也未实现正式紧凑文本、短引用解析或跨命令失效规则。
...
当前决定：首版使用 CLI 与按会话运行的 Mac 宿主进程，保持设备会话复用，优先 CoreDevice 直连并尽量少依赖第三方工具；Runner 尽量薄且可被替换。设备通信和最小宿主已实现，完整观察与动作接口按后续计划接入。
...
动作失效的宿主侧实现建议：在同一设备的串行临界区内，先校验并解析引用，将它置于不可复用的待定状态，再向后端派发。动作已派发或结果未知时旧引用失效；只有能确认未派发且没有其他失效原因时才解除待定状态。宿主进程重启后不继承旧引用的有效性，也不自动重放未完成请求。这样无需 Runner 实现面向 agent 的引用协议，仍能覆盖“点击已执行但调用方未收到响应”的情况。具体实现未定稿或验证。
